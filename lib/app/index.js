...
module.exports = function render(state, prev, send) {
        ...
        if (isImportant) {
          ...
          if (next && next < state.now && date < state.now) {
            ...
            let index = days.findIndex(day => {
              return moment(day.date).isSame(date) || new Date(day.date) > date;
            });
          ...
          if (day) {
            ...
            const index = day.items.findIndex(item => new Date(item.date) >= prio);

            // Add this in place
            day.items.splice(index === -1 ? 0 : index, 0, Object.assign({ render }, todo));
          } else {
            // Find the following day's index or fallback to first index
            const index = days.length ? days.findIndex(day => new Date(day.date) >= prio) : 0;

            // Create a new entry if there's nothing on the same date
            day = { date, isTodo: true, items: [ Object.assign({ render }, todo) ], title: lang.UPCOMING };

            // Inject day in days before the next date
            days.splice(index > 0 ? index - 1 : 0, 0, day);
          }
        }
      });
  }
  ...
  return html`
    <div class="App">
      ...
      <ol>
        ${ onlyif(showNotifications || important.length, html`
          <li>

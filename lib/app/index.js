...
const noresult = require('./noresult');
...
module.exports = function render(state, prev, send) {
  ...
  return html`
    <div class="App">
      ...
      ${ banner(state, prev, send) }
      ${ search(state, prev, send) }
      ${ filter(state, prev, send) }

      ${ onlyif(state.search && !days.length, noresult(state, prev, send)) }

      <ol class="List">
        ${ onlyif((showNotifications && unreadNotifications.length) || important.length, html`
          <li class="List-section">
            <h3 class="List-heading">
              <span class="List-icon">${ exclamation() }</span>
              ${ lang.IMPORTANT }
              <sup class="List-badge">
                ${ important.length + (showNotifications ? unreadNotifications.length : 0) }
              </sup>
            </h3>
            <ol class="List">

...
module.exports = function (props, state, send) {
  ...
  for (let item of props.items) {
    if (item.type === type && item.expandable) {
  ...
  return html`
    <li class="List-section">
      <h3 class="List-heading">
        <span class="List-icon">${ icons[props.icon || 'calendar']() }</span>
        <time>${ props.title || moment(props.date).calendar(state.now, format(state.now)) }</time>
      </h3>
      <ol class="List">
        ${ groups.map(group => {
...
function expandable(group, state, send) {
  ...
  const onclick = () => send('list:toggle', { id, expand: !isExpanded });

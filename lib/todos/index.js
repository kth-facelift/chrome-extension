...
exports.assignment = function (props, state, send) {
  ...
  return html`
    <article class="List-content List-content--todo" id=${ `todo_${ props.id }` }>
...
exports.task = function (props, state, send) {
  return html`
    <article class="List-content List-content--todo" id=${ `todo_${ props.id }` }>
      ...
      <div class="List-actions">
        <button class="List-action List-action--dismiss" onclick=${ dismiss(props.id, send) } title=${ lang.DISMISS }>✕</button>
        <button class="List-action" onclick=${ editTask(props, send) } title=${ lang.EDIT_TODO }>${ pencil(14) }</button>
      </div>
    </article>
  `;
};

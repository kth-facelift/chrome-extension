...
module.exports = function (props, now, send) {
  return html`
    <article class="List-content List-content--notification" id=${ `notification_${ props.id }` }>
      <h4 class="List-title">${ lang[(props.type || 'NOTIFICATION').replace(/\s+/g, '_').toUpperCase()] }</h4>

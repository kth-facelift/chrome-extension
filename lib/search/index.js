    ...
    modal.render(html`
      <form onsubmit=${ onadd } class="Form">
        ...
        <fieldset>
          ...
          <label class="Form-label">
            <input type="radio" value="0" name="priority" checked onchange=${ onchange } />
            ${ lang.NO_PRIORITY }
          </label>
          <label class="Form-label">
            <input type="radio" value="1" name="priority" onchange=${ onchange } />
            ${ lang.LOW_PRIORITY }
          </label>
          <label class="Form-label">
            <input type="radio" value="2" name="priority" onchange=${ onchange } />
            ${ lang.MEDIUM_PRIORITY }
          </label>
          <label class="Form-label">
            <input type="radio" value="3" name="priority" onchange=${ onchange } />
            ${ lang.HIGH_PRIORITY }
          </label>
        </fieldset>

        <button type="submit" class="Form-button">${ lang.ADD_TODO }</button>
        <button type="button" class="Form-button Form-button--cancel" onclick=${ onclose } formnovalidate>${ lang.CANCEL }</button>
      </form>
    `);
